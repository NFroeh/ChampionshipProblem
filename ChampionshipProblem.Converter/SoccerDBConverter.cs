        ...
        public static void ConvertEuropeanSoccerDb()
        {
            ...
            using (EuropeanSoccerEntities europeanDb = new EuropeanSoccerEntities())
            {
                ...
                List<Scheme.League> leagues = europeanDb
                    .Leagues
                    .ToList();
                leagueDictionary = leagues.ToDictionary((l) => l.id, (l) => string.Join(" ", l.name.Split(' ').Skip(1)));

                foreach (Scheme.League league in leagues)
                {
                    string leagueName = string.Join(" ", league.name.Split(' ').Skip(1)); 
            ...
            using (EuropeanSoccerEntities europeanSoccerEntities = new EuropeanSoccerEntities())
            {
                ...
                foreach (Scheme.Match oldMatch in oldMatches)
                {
                    Classes.Match newMatch = new Classes.Match()
                    {
                        ...
                        LeagueId = newLeagues
                                    .Single((league) => league.Name == leagueDictionary[oldMatch.league_id.Value] && 
                                            (Classes.Country)Enum.Parse(typeof(Classes.Country), oldMatch.Country.name) == league.Country).Id,
                        HomeId = newTeams
                                    .Single((team) => team.Name == teamDictionary[oldMatch.home_team_api_id.Value]).Id,
                        AwayId = newTeams
                                    .Single((team) => team.Name == teamDictionary[oldMatch.away_team_api_id.Value]).Id,
